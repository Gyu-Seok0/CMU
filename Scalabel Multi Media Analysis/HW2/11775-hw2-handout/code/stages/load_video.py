...
class LoadVideo(Stage):
    ...
    def downsample_frames(self, frames: np.ndarray, frame_rate: float):
        ...
        shape0 = int(frames.shape[0] // self.target_frame_rate) # 칸
        shape1,shape2,shape3 = frames.shape[1:]
        down_frames = frames[::shape0] 
        #np.resize(frames, (shape0,shape1,shape2,shape3))
        return down_frames
    ...
    def process(self, task):
        ...
        if str(video_id + ".pkl") in os.listdir("/home/gyuseok/CMU/11775-hw2-handout/data/cnn3d"):
            task.finish()

...
class SIFTFeature(Stage):
    ...
    def extract_sift_feature(self, frame: np.ndarray) -> np.ndarray:
        ...
        image8bit = cv2.normalize(frame, None, 0, 255, cv2.NORM_MINMAX).astype('uint8')
        _, feature = self.sift.detectAndCompute(image8bit, None)

        if feature is None:
            return np.zeros((self.num_features, 128))

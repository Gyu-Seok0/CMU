...
class CNNFeature(Stage):
    ...
    def allocate_resource(self, resources, *, model_name='convnext_base', #'resnet18',
                          weight_name='ConvNeXt_Base_Weights', #'ResNet18_Weights',
    ...
    def extract_cnn_features(self, frames: torch.Tensor) -> torch.Tensor:
        ...
        new_frames = frames.permute(0, 3, 1, 2)
        # Then apply self.transforms to batch to get model input.
        trans_frames = self.transforms(new_frames)

        # Finally apply self.model on the input to get features. 
        # Wrap the model with torch.no_grad() to avoid OOM.
        with torch.no_grad():
            trans_frames = trans_frames.to(self.device)
            features = self.model(trans_frames)
            
        return features["feature"].reshape(B,-1)

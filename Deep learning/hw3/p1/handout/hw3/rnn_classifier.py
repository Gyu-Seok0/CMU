    ...
    def forward(self, x, h_0=None):
        ...
        for seq in range(seq_len):
            ...
            for layer in range(self.num_layers):
                if layer == 0:
                    h_t = self.rnn[layer].forward(x[:,seq,:], self.hiddens[seq][layer])
                else:
                    h_t = self.rnn[layer].forward(seq_hidden[-1][0], self.hiddens[seq][layer])
                seq_hidden.append([h_t])
            
            seq_hidden = np.concatenate(seq_hidden, axis = 0)
            self.hiddens.append(seq_hidden)
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for time in range(seq_len - 1, -1, -1):
            for layer in range(self.num_layers - 1, -1, -1):
                # backprop
                if layer == 0:
                    dx, dh_prev_t = self.rnn[layer].backward(dh[layer], self.hiddens[time + 1][layer],self.x[:,time,:], self.hiddens[time][layer])
                else:
                    dx, dh_prev_t = self.rnn[layer].backward(dh[layer], self.hiddens[time + 1][layer],self.hiddens[time + 1][layer-1], self.hiddens[time][layer])
                
                # update
                dh[layer] = dh_prev_t
                
                if time == seq_len - 1 and layer > 0:
                    dh[layer - 1] = dx
                
                elif time < seq_len - 1 and layer > 0:
                    dh[layer - 1] += dx

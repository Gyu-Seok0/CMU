...
def get_ngrams(list_of_words, n):
    ...
    """
    n_grams = []
    for start in range(len(list_of_words) - n + 1):
        n_grams.append(tuple(list_of_words[start:start+n]))
...
class LanguageModel():
    ...
    def get_smooth_probabilites(self,n_gram):
        ...
        """
        result = {}
        for key, value in self.n_grams_counts.items():
          if self.n == 1:
             result[key] = (value + self.alpha) / (sum(self.n_grams_counts.values()) + self.alpha * len(self.vocab)) 

          else:
            target = key[:-1]
            denominator = self.prefix_counts[target]
            result[key] = (value + self.alpha) / (denominator + self.alpha * len(self.vocab))

        #self.denominator = denominator + self.alpha * len(target)
        self.denominator = self.alpha * len(self.vocab)
        return result
...
def perplexity(lm, test_data):
    ...
    for n_gram in n_grams:
      try:
        likelihood += math.log(gram2prob[n_gram])
      except:
        if n_gram[:-1] in lm.prefix_counts.keys():
          likelihood += math.log(lm.alpha / (len(lm.vocab)*lm.alpha  + lm.prefix_counts[n_gram[:-1]]))
        else:
          likelihood += math.log(1/len(lm.vocab))

    perplexity = math.exp(-likelihood/len(n_grams))
    return perplexity

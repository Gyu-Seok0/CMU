...
class R_MAPPO():
    ...
    def cal_value_loss(self, values, return_batch, active_masks_batch):
        ...
        mse = mse_loss(errors)
        critic_loss = (mse * active_masks_batch).sum() / active_masks_batch.sum()
        return critic_loss
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        if(self.use_KL_pen):
          ...
          # 수정요함.
          L_KLPEN = kl_divergence(FixedCategorical(torch.exp(action_log_probs)), FixedCategorical(torch.exp(old_action_log_probs_batch)))
          L_KLPEN = imp_weights*adv_targ - self.beta * L_KLPEN 
          print("L_KLPEN", L_KLPEN)
          ...
          L_CLIP =  torch.min(L_CLIP, imp_weights * adv_targ)
          if self._use_policy_active_masks:
        ...
        critic_loss = self.cal_value_loss(values, return_batch, active_masks_batch)
        (critic_loss * self.value_loss_coef).backward()

        self.policy.critic_optimizer.step()


        return critic_loss, actor_loss, imp_weights

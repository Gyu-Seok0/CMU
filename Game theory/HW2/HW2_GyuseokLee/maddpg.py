...
def construct_q_function(agent_index, obs_n, act_n):
    ...
    '''
    # TODO
    #construct_q_function(q_index, obs_ph_n, act_ph_n)
...
def compute_q_targets(agent_index, rew, done, obs_next_n, target_policy_func_n, target_q_func, gamma):
    ...
    target_a_next = [target_policy_func_n[i](obs_next_n[i]) for i in range(len(obs_next_n))]
    target_q_next = target_q_func(*(obs_next_n + target_a_next))

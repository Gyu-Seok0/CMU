    ...
    def __init__(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            agent.color = [[0.85, 0.35, 0.35],
                    [0.35, 0.35, 0.85], 
                    [0.35, 0.85, 0.35]][i]
            agent.poacher_caught_rew = [-1, 1, 1][i] # poacher loss
            agent.animal_caught_rew = [1.25, -5, -5][i] # poacher get
            agent.caught = False

            # addition
            #agent.penalty = [-0.005, -0.025, -0.025][i]
            agent.dist = np.inf
            #angent.visible_reward = []
            
            # condier the distance 
            agent.dis_minimize_penalty = [-0.0005, 0.0015, 0.0015][i]
            agent.dis_maximize_penalty = [0.0005, -0.0015, -0.0015][i]
            agent.dis_reward = None

            if i == 0:
                agent.landmark_collision = 0
    ...
    def step(self):
        ...
        for i, landmark in enumerate(self.landmarks):
            if self.is_collision(self.poacher, landmark):
                ...
                self.poacher.landmark_collision += 0.35
            else:
                landmark.poacher_collision_time = 0
                self.poacher.landmark_collision -= 0.00115
...
class Scenario(BaseScenario):
    ...
    def reward(self, agent, world):
        ...
        if world.was_poacher_caught:
            ...
            if agent.name == "poacher":
                return 0.99 * agent.dis_reward + agent.landmark_collision * 0.01
            else:
                return agent.dis_reward
    ...
    def is_visible(self, agent, target):
        ...
        if dist < agent.dist:
            agent.dis_reward = agent.dis_minimize_penalty
            target.dis_reward = target.dis_minimize_penalty
        else:
            agent.dis_reward = agent.dis_maximize_penalty
            target.dis_reward = agent.dis_maximize_penalty

        if dist < agent.sight:
            agent.dis_reward = agent.dis_minimize_penalty * 5
            target.dis_reward = target.dis_minimize_penalty * 5
